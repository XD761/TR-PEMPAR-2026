...
> **Isi bagian ini dengan data Anda sebelum dikumpulkan** — nama, NIM, kelas.
...
## 2. Deskripsi Singkat
Program ini adalah simulasi api 2D berbasis particle system dengan 3000 partikel
aktif secara bersamaan (jauh di atas syarat minimum 500). Aplikasi berbasis
window (SDL2), bukan konsol, dan berjalan real-time (~60 FPS). Paralelisme
diimplementasikan dengan **MPI** (Message Passing Interface).
...
|---:|---:|---:|
| 1 | 0.0537 ms | 0.0080 ms |
| 2 | 0.0261 ms | 0.3443 ms |
| 4 | 0.0131 ms | 0.5248 ms |
| 8 | 0.0073 ms | 0.6794 ms |
...
- **Keterbatasan pengujian ini**: sandbox tempat kode ini diverifikasi hanya
  ...
  wall-clock total (`time mpirun -np N ./fire_sim 600`) untuk N = 1, 2, 4, 8
...
SDL_VIDEODRIVER=dummy mpirun -np 8 ./fire_sim 300
```
