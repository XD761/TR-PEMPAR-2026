...
#define WIDTH            800
#define HEIGHT           600
#define N_PARTICLES      3000      /* total partikel aktif (>= 500)   */
#define GRID_COLS        40        /* jumlah bucket untuk histogram   */
#define TARGET_FPS       120
#define FIXED_DT          (1.0f / TARGET_FPS)

#define EMITTER_HALF_WIDTH   90.0f
#define BASE_BUOYANCY         55.0f
#define DENSITY_BUOYANCY_GAIN  6.0f
#define TURBULENCE_STRENGTH   40.0f
#define VX_DAMPING             0.965f
#define LIFE_MIN               0.9f
#define LIFE_MAX               1.6f

#define MOUSE_REPEL_RADIUS    130.0f  /* jangkauan pengaruh mouse (px)     */
#define MOUSE_REPEL_STRENGTH 1400.0f  /* kekuatan gaya tolak                */
...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        window = SDL_CreateWindow("Simulasi Api - MPI Particle System",
    ...
    while (!quit) {
        ...
        if (rank == 0) {
            ...
            snprintf(title, sizeof(title),
                     "Simulasi Api - MPI Particle System | Intensitas: %.2f%s",
        ...
        for (int i = 0; i < my_count; i++) {
            ...
            float buoyancy = (BASE_BUOYANCY + DENSITY_BUOYANCY_GAIN * (density / 30.0f))
                             * control.intensity;
            float turbulence = sinf(sim_time * 3.0f + (float)(p->seed % 1000) * 0.01f)
                                * TURBULENCE_STRENGTH;

            p->vx = (p->vx + turbulence * FIXED_DT) * VX_DAMPING;
